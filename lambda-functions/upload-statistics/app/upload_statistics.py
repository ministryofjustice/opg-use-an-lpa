...
class StatisticsCollector:
    ...
    def __init__(self):
        ...
        self.dynamodb_table_prefix = (
            f"{self.environment}-" if self.environment != "local" else ""
        )
    ...
    def iterate_months(self):
        ...
        while True:
            ...
            yield current
            if (
                current.month == self.enddate.month
                and current.year == self.enddate.year
            ):
                break
            else:
    ...
    def paginated_get_total_counts_by_month(
        self, month_start, month_end, table_name, filter_exp
    ):
        running_sum = 0
    ...
    def sum_metrics(self, event):
        ...
        for month_start in self.iterate_months():
            ...
            datapoints = self.get_metric_statistic(
                self.format_month(month_start), self.format_month(month_end), event
            )
    ...
    def update_dynamodb_totals(self, latest_month):
        previous_month = (
            datetime.datetime.strptime(latest_month, "%Y-%m") - relativedelta(months=1)
        ).strftime("%Y-%m")
        ...
        try:
            previous_month_response = self.aws_dynamodb_client.get_item(
                TableName=f"{self.dynamodb_table_prefix}Stats",
                Key={"TimePeriod": {"S": previous_month}},
            )
        except Exception:
...
def lambda_handler(event, context):
    ...
    return {"message": f"Success - {str(success)}"}
