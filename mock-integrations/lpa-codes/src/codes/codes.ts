...
function expireCode(paperVerificationCode: string, reason: ExpiryReason): LpaCode|null {
  logger.debug('Expired code ' + reason)

  let code = getCode(paperVerificationCode)
  if (code === null) {
    return null
  }
  ...
  code.expiry_date = Math.floor(expiry / 1000)
  code.expiry_reason = ExpiryReason[reason]
