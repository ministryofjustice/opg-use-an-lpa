...
# Setup dependencies
uv sync --locked --directory ../../../scripts/call-api-gateway

for n in $(cat list.txt )
do
    echo "Working on $n..."
    example_name="lpa${n: -4:4}"
    lpa_yaml_data=$(aws-vault exec identity -- uv run --directory ../../../scripts/call-api-gateway python call_api_gateway.py $n | yq -P)
