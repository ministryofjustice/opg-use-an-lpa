...
class SiriusLpas extends AbstractApiClient implements LpasInterface, RequestLetterInterface
{
    ...
    public function requestLetter(LpaUid $caseId, ?string $actorId, ?string $additionalInfo): void
    {
        if ($caseId->getLpaSource() === LpaSource::LPASTORE) {
            $entry = [
                'Source'       => 'opg.poas.use',
                'DetailType'   => $actorId === null ? 'request-cleanse' : 'request-letter',
                'Detail'       => json_encode(
                    $actorId === null
                        ? ['uid' => $caseId->getLpaUid(), 'notes' => $additionalInfo]
                        : ['uid' => $caseId->getLpaUid(), 'actor' => $actorId]
                ),
            ...
            if ($result->get('FailedEntryCount') > 0) {
                $this->logger->warning(
                    'Failed to put event for LPA {lpaUid}',
                    [
