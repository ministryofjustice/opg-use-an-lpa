...
if ! [[ -z "${AWS_ENDPOINT_DYNAMODB}" ]]; then
    ...
    --global-secondary-indexes \
      ...
      IndexName=NewEmailIndex,KeySchema=["{AttributeName=NewEmail,KeyType=HASH}"],Projection="{ProjectionType=KEYS_ONLY}",ProvisionedThroughput="{ReadCapacityUnits=10,WriteCapacityUnits=10}" \
