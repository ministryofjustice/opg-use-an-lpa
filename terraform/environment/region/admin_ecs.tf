...
resource "aws_security_group_rule" "admin_ecs_service_ingress" {
  ...
  source_security_group_id = var.shared_admin_load_balancer_enabled ? tolist(data.aws_lb.shared_admin[0].security_groups)[0] : aws_security_group.admin_loadbalancer[0].id
  lifecycle {
    create_before_destroy = true
  }
