...
resource "aws_security_group_rule" "mock_onelogin_ecs_service_ingress" {
  ...
  source_security_group_id = var.shared_load_balancers_enabled ? tolist(data.aws_lb.shared_mock_onelogin[0].security_groups)[0] : aws_security_group.mock_onelogin_loadbalancer[0].id
  lifecycle {
    create_before_destroy = true
  }
