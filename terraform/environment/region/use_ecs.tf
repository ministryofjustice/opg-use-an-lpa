...
resource "aws_security_group_rule" "use_ecs_service_ingress" {
  ...
  source_security_group_id = var.shared_load_balancers_enabled ? tolist(data.aws_lb.shared_actor[0].security_groups)[0] : aws_security_group.use_loadbalancer[0].id
  lifecycle {
    create_before_destroy = true
  }
